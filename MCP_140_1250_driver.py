...
##################Output Voltage###############
    def set_output_voltage(self, voltage):
        """set output voltage"""
        self._send_command('>S0 '+str(voltage))
        return self._get_data()

    def get_output_voltage(self):
        """Query the actual output voltage of the power supply."""
        self._send_command('>M0?')
        return self._get_data()
##################RampRate Voltage###############

    def set_ramprate_voltage(self, voltage):
        """set ramprate voltage"""
        command='>S0R '
        self._send_command(command+str(voltage))

    def get_ramprate_voltage(self):
        """get output voltage"""
        command='>S0R'
        response=self._send_command(command+' ?')
        return response[int(len(command)+1):int(len(response))]

    def get_ramprate_behave(self):
        """get ramprate behave"""
        command='>S0B?'
        response=self._send_command(command)
        return response
#######################Switch On Off########################
    ...
    def switch_on_ps(self):
        """switch on power supply"""
        self._send_command('F1')
        return self._get_data()

    def switch_off_ps(self):
        """switch on power supply"""
